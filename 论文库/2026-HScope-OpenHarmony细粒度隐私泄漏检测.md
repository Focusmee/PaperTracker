...
research_type: 应用型
title: HScope：OpenHarmony应用细粒度隐私泄漏检测
english_title: "Fine-Grained Privacy Leakage Detection in OpenHarmony Apps"
authors: [Aohan Mei, Guangliang Yang, Xinming Guo, Yi Wang, Fuan Gui, Min Yang]
venue: ACM SIGSOFT International Symposium on Software Testing and Analysis (ISSTA 2026)
publication_date: 2026-10
doi: 10.1145/3832296
official_url: https://conf.researchr.org/details/issta-2026/issta-2026-research-papers/205/Fine-Grained-Privacy-Leakage-Detection-in-OpenHarmony-Apps
code_url: https://github.com/FDU-SSR/lian
local_pdf: "[[附件/论文原文/HScope-英文原文-ISSTA-2026.pdf]]"
translation_pdf: "[[附件/论文译文/HScope-中文精读批注版.pdf]]"
publication_status: 官方已接收（2026年10月出版）
status: unread
domain: [移动应用隐私, 静态污点分析, OpenHarmony]
parent: "[[README]]"
related: ["[[论文库/2026-DIULENS-移动CMP隐私风险]]", "[[论文库/2026-PINFINDER-SDK隐私上下文一致性]]", "[[论文库/2025-IRIS-LLM辅助静态漏洞分析]]"]
sources: [https://conf.researchr.org/details/issta-2026/issta-2026-research-papers/205/Fine-Grained-Privacy-Leakage-Detection-in-OpenHarmony-Apps, https://yian-lang.net/guangliang/assets/2026_issta_OpenHarmony_security.pdf, https://doi.org/10.1145/3832296, https://github.com/FDU-SSR/lian]
created: 2026-08-24
updated: 2026-08-31
category: [移动隐私, 字节码分析, 污点分析]
...
demo_status: 未计划
next_action: 用25分钟阅读图1、图2和表1，把一个OAID从source到http.post sink的传播路径画成五个节点
tags: [研究/移动隐私, 方法/静态分析, 方法/程序分析]
---

# HScope：OpenHarmony应用细粒度隐私泄漏检测

## 本地原文

- [[HScope-英文原文-ISSTA-2026.pdf]]：作者公开 PDF，22 页；题名和作者已核对。

> [!abstract] 一句话结论
> HScope 直接分析 OpenHarmony 应用的无类型 Ark 字节码，通过抽象解释、边分析边构建调用图、框架专用 ICC 建模和静态污点传播，判断位置、设备标识符等敏感数据能否到达网络或日志等泄漏出口。

## 关键图像入口

![[附件/论文截图/HScope-图2-系统工作流.png]]

**系统工作流：** 优先读清字节码输入、组件/调用关系恢复、污点分析与结果验证的先后依赖。

![[附件/论文截图/HScope-图1-跨组件泄漏案例.png]]

**跨组件案例：** 用它核对普通单文件分析为何会断链，以及 HScope 新建的关系具体补上哪一段。

## 为什么与当前研究相关

### 来源事实

论文已由 ISSTA 2026 官方页面列为研究论文；作者公开了 22 页正文、DOI 和实现入口。正文说明 HScope 输入应用包及可配置 source/sink 策略，输出潜在隐私泄漏路径；它不需要应用源代码，也没有使用 LLM。

### 我的理解

它正好补齐 DIULENS/PICOSCAN 学习中的“静态分析到底怎样落地”：不是把整份反编译代码交给模型猜，而是先把字节码恢复成中间表示，再用明确传播规则计算 `敏感数据源 → 变量/对象/回调/组件 → 泄漏出口`。这种结果可作为动态 GUI 探索前的候选路径和确定性证据。

### 与 DIULENS / PICOSCAN 的关系

- 与 [[论文库/2026-DIULENS-移动CMP隐私风险]]：DIULENS 关注同意界面、用户选择与运行时事件是否一致；HScope 关注所有潜在可达的数据流。前者回答“同意是否有效”，后者回答“数据可能流到哪里”。
- 与 [[论文库/2026-PINFINDER-SDK隐私上下文一致性]]：两者都需要隐私 API 知识。HScope 把 API 分成 source 和 sink 并做污点传播；PINFINDER 还要判断 SDK 调用的业务上下文和责任归属。
- 对当前 Demo 的迁移点：先用简化 source/sink 规则生成静态路径，再让 Appium 只探索与这些路径相关的按钮或页面，最后用网络/Hook 事件验证路径是否实际发生。

## 研究问题、输入与输出

- 研究问题：如何在缺少类型信息、调用大量间接回调且组件可共享对象引用的 Ark 字节码上，准确且可扩展地发现隐私泄漏？
- 输入：OpenHarmony 应用包，以及 JSON 格式的敏感 source API、危险 sink API 和污点标签策略。
- 输出：潜在泄漏报告，以及从 source 到 sink 的上下文敏感数据流路径。
- 威胁模型：应用读取敏感信息后，经网络、日志或短信等通道泄露；分析目标是程序中可能发生的流，不等于证明真实用户设备上已经发生泄漏。

## 系统完整工作链路与分工

```text
OpenHarmony应用包 + source/sink JSON策略
  ↓ ark_disasm
Ark字节码文本
  ↓ 解析、结构化反编译、变量/函数名恢复
轻量IR
  ↓ 抽象解释：计算变量、对象、函数和闭包的抽象内存对象
语义状态 + 边分析边构建的调用图
  ↓ OpenHarmony框架建模：回调、Ability、Router、共享UI状态
跨函数、跨组件的控制流与数据流
  ↓ 上下文敏感、流敏感的污点传播
source → 中间对象/回调 → sink 路径
  ↓ 人工确认候选路径是否真实可行
已确认隐私问题
```

| 部分 | 在 HScope 中承担什么 | 不承担什么 |
|---|---|---|
| 静态分析 | 完成字节码解析、语义近似、调用目标恢复、ICC 建模和污点传播 | 不保证被报告路径在某次真实运行中一定触发 |
| 动态分析 | 原系统主流程没有动态执行 App | 不记录点击、网络包或同意时序 |
| LLM / Agent | 原系统不使用 | 不能把 LLM 解释当成 HScope 的检测证据 |
| 知识库/策略 | JSON 中维护 source、sink、敏感类别和框架模型；不是 SDK 名单的简单集合 | 未收录 API 或未建模框架语义会形成盲区 |
| 人工验证 | 作者逐条检查真实 App 候选流，确认 48 条中的 39 条可行 | 人工确认规模有限，且不是自动化 Ground Truth 生成器 |

## 零基础术语与具体例子

- **Source（污点源）**：敏感数据首次进入程序的位置。例如 `identifier.getOAID()` 返回设备广告标识符。
- **Sink（污点汇）**：数据可能离开安全边界的位置。例如 `http.post(data)` 把数据发给服务器。
- **污点分析**：给敏感值贴上标签并追踪标签传播。若 `oaid → message → callback参数 → http.post`，就报告一条候选泄漏。
- **中间表示（IR）**：把复杂字节码翻译成更统一的赋值、读写属性和函数调用等指令，方便后续算法处理。
- **抽象解释**：不真正运行每一种输入，而是用有限的抽象对象近似所有可能状态。例如把“在代码位置 L 创建的一类对象”表示成一个抽象位置。
- **间接调用**：调用目标不是代码中直接写死的函数名。例如子组件执行 `this.callback()`，实际调用父组件传来的匿名函数。

## 推荐阅读顺序

1. 第 2.2 节与图1：先追踪 OAID 如何跨组件进入 `http.post`。
2. 图2：记住“字节码前端—语义分析—隐私检测”三段式总流程。
3. 第 3.1 节和图3：理解字节码为什么要先变成 IR。
4. 第 3.2–3.4 节：先看文字和示例，首次阅读可略过公式证明细节。
5. 第 3.5–3.6 节、表1和表2：掌握 source/sink 与污点传播规则。
6. 表3、表4和第 4.6 节：区分微基准准确性、真实 App 发现和基线比较。
7. 第 6 节：用局限反推静态分析为什么会误报、漏报。

## 已核验的数据集、基线、指标与关键数字

- OH-Bench：50 个测试用例、11 类模式；HScope 的 Precision 和 Recall 均为 87.5%，F1 为 0.88。
- 基线 ArkAnalyzer 在同一微基准上的 Precision 为 81.8%、Recall 为 22.5%、F1 为 0.35。
- 真实数据集：300 个近期开源 OpenHarmony 应用；作者解释官方应用市场包受加密和反嗅探限制，因此没有直接抓取商店闭源包。
- HScope 报告 48 条潜在泄漏，人工确认其中 39 条可行，涉及 27 个 App；这不是“39/48 的通用准确率”，因为样本来自特定数据集和人工判定流程。
- 平均每个 App 分析 206 秒、占用 1,523 MB 内存；实验机为 i7-14700、32 GB RAM、Ubuntu 24.04。
- HScope 原型约 33,757 行 Python，并用 1,252 行 JavaScript 定义 tree-sitter 语法；论文版本已归档，当前实现集成在 Lian 中，可用 `--lang abc` 或 `-l abc` 激活。

## 局限和复现条件

- 反射、字符串动态导入和运行时生成属性仍可能造成漏报。
- 对 Map/Record 等复合结构采用汇总近似，一个字段被污染可能导致兄弟字段也被标记，形成误报。
- 有界上下文敏感度可能无法完全解析深层嵌套或高度多态的回调。
- 复现需要 Ubuntu、OpenHarmony/ArkCompiler 的 `ark_disasm`、论文对应 Lian/HScope 版本、应用包及 source/sink 策略；完整 300 App 数据集能否直接获取仍需查看 Zenodo 归档内容。
- 静态报告必须与人工代码检查或动态网络/Hook 证据结合，不能把“可达路径”直接表述为“已经发生的数据泄漏”。

## 三分钟课堂提纲

1. **问题（30秒）：** OpenHarmony 的 Ark 字节码无类型、回调动态、组件共享状态，Android 分析器直接搬来会漏掉路径。
2. **方法（80秒）：** 字节码转 IR；抽象解释恢复对象和调用；框架模型连接回调与组件；污点分析寻找 source 到 sink。
3. **例子（40秒）：** OAID 写入父子组件共享状态，经子组件 callback 回到父组件，最后进入 `http.post`。
4. **评估（20秒）：** 50 个微用例 F1 0.88；300 个 App 中人工确认 39 条泄漏路径。
5. **边界（10秒）：** 静态可达不等于真实发生，反射和复合对象近似仍会漏报或误报。

## 最小 Demo

先不要复刻 3 万多行 HScope。写一个 50–100 行的“教学污点分析器”：

1. 输入一段固定的伪 ArkTS 代码和一份 `sources/sinks.json`。
2. 只支持赋值、属性写入、属性读取和函数参数四条传播规则。
3. 给 `identifier.getOAID()` 返回值贴 `OAID` 标签。
4. 输出 `getOAID → C1.message → C2.message → callback(msg) → http.post` 路径。
5. 再删除“共享属性别名”规则，让路径消失，用对比说明 HScope 的框架建模价值。

这个 Demo 展示的是论文核心原理，不声称复现 HScope 的 Ark 字节码前端、完整抽象解释或实验结果。

## 主动回忆题

1. 为什么 OpenHarmony 的无类型 Ark 字节码使预先构建完整调用图很困难？
2. source/sink 列表与 SDK 名单有什么区别？
3. 为什么父组件和子组件共享同一抽象位置后，污点能够在没有显式赋值的情况下传播？
4. 静态分析报告 48 条候选而人工确认 39 条，剩余差异可能来自哪些近似？
5. HScope 与 DIULENS 各自能看到什么证据，为什么二者不能互相替代？

## 下一步

用 25 分钟阅读图1、图2和表1，手画 `OAID source → 共享状态 → callback → http.post sink` 五节点路径，并在每条边旁标注所需传播规则。

## 一手来源

- [ISSTA 2026 官方论文页](https://conf.researchr.org/details/issta-2026/issta-2026-research-papers/205/Fine-Grained-Privacy-Leakage-Detection-in-OpenHarmony-Apps)
- [作者公开论文 PDF](https://yian-lang.net/guangliang/assets/2026_issta_OpenHarmony_security.pdf)
- [DOI](https://doi.org/10.1145/3832296)
- [Lian / HScope GitHub](https://github.com/FDU-SSR/lian)
